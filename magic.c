...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
    send_to_char( "Pray which what where?\n\r", ch );
    return;
    }
    ...
    if ( arg2[0] == '\0' )
    {
        ...
        {
        send_to_char( "Pray the prayer on whom?\n\r", ch );
        return;
        }
    }
    else
...
/*      if ( !IS_NPC(victim) && (!IS_SET(ch->act,PLR_IC) || !IS_SET(victim->act,PLR_IC)))
        {
            send_to_char( "You and your victim must both be in
IC mode to fight.\n\r", ch );
            return;
        }
*/
    ...
    {
        send_to_char( "You cannot pray this prayer on another.\n\r", ch );
        return;
    }
    ...
    if ( arg2[0] == '\0' )
    {
        send_to_char( "What should the spell be prayed upon?\n\r", ch );
        return;
    }
    ...
    if (arg2[0] == '\0')
    {
        if ((victim = ch->fighting) == NULL)
        {
        send_to_char("Pray the prayer on whom or what?\n\r",ch);
        return;
        }
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (check_dispel(level,victim,skill_lookup("change alignment")))
    {
        found = TRUE;
        act("$n's aura returns to normal.",victim,NULL,NULL,TO_ROOM);
    }
    ...
    if (check_dispel(level,victim,skill_lookup("radiant aura")))
    {
        act("$n's holy aura fades away.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    if (check_dispel(level,victim,skill_lookup("dark aura")))
    {
        act("$n's dark aura fades away.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
...
void spell_create_food( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    act( "$p sprouts and grows from nothingness.", ch, mushroom, NULL, TO_ROOM );
    act( "$p sprouts and grows from nothingness.", ch, mushroom, NULL, TO_CHAR );
    return;
}
...
void spell_cure_poison( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (check_dispel(level+10,victim,gsn_poison))
    {
...
void spell_fireball( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    dam /= 2;
    damage( ch, victim, dam, sn, DAM_FIRE ,TRUE);
    fire_effect(victim,level,dam/4,TARGET_CHAR);
    return;
}
...
void spell_invigorate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    victim->move = UMIN( victim->move + (level * (get_attribute(ch,STAMINA) + ch->csabilities[CSABIL_THEOLOGY])), victim->max_move );

    if (victim->max_move == victim->move)
        send_to_char("You feel completely refreshed!\n\r",victim);
    else
        send_to_char( "Your body feels invigorated and refreshed.\n\r", victim );
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_agg_damage( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;
    ...
    if (obj->item_type == ITEM_WEAPON)
    {
        ...
        ||  IS_OBJ_STAT(obj,ITEM_BLESS) || IS_OBJ_STAT(obj,ITEM_BURN_PROOF))
        {
        act("You can't seem to grant $p with aggravated damage.",ch,obj,NULL,TO_CHAR);
        return;
        }

        if (IS_WEAPON_STAT(obj,WEAPON_AGG_DAMAGE))
        {
        act("$p already has aggravated damage.",ch,obj,NULL,TO_CHAR);
        return;
        }
        ...
        act("$p is imbued with aggravated damage.",ch,obj,NULL,TO_ALL);
        return;
    }

    act("You can't do that to $p.",ch,obj,NULL,TO_CHAR);
    return;
    }
    ...
    af.bitvector = 0;
    affect_join( victim, &af );
    send_to_char( "You scream in pain!\n\r", victim );
    act("$n screams in pain!",victim,NULL,NULL,TO_ROOM);
    return;
}
...
void spell_remove_curse( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (check_dispel(level+10,victim,gsn_curse))
    {
    send_to_char("You feel better.\n\r",victim);
...
void spell_lightning_fist(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    {
        act("$n takes a wild swing at $N and misses!",ch,NULL,victim, TO_ROOM);
        act("You take a wild swing at $N with your {WLightning Fist{x and miss!.",ch,NULL,victim, TO_CHAR);
        return;
    }


    dam = level*number_range(1,10);
    if(is_affected(ch,gsn_shockshield)) dam += 100;
    if(dam < 100) {
    act("$n lightly hits $N with a {WLightning Fist{x.",ch,NULL,victim, TO_ROOM);
    act("You lightly hit $N with your {WLightning Fist{x.",ch,NULL,victim, TO_CHAR);
    }

    else if(dam < 300) {
    act("$n {rhits{x $N with a {WLightning Fist{x.",ch,NULL,victim, TO_ROOM);
    act("You {rhit{x $N with your {WLightning Fist{x.",ch,NULL,victim, TO_CHAR);
    }

    else if(dam < 500) {
    act("$n {Rsmashes{x $N with a {WLightning Fist{x.",ch,NULL,victim, TO_ROOM);
    act("You {Rsmashes{x $N with your {WLightning Fist{x.",ch,NULL,victim, TO_CHAR);
    }

    else {
    act("$n {RC{YR{RU{YS{RH{YE{RS{x $N with a {WLightning Fist{x.",ch,NULL,victim, TO_ROOM);
    act("You {RC{YR{RU{YS{RH{x $N with your {WLightning Fist{x.",ch,NULL,victim, TO_CHAR);
    }
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    dam = UMAX(hp_dam + dice_dam/10,dice_dam + hp_dam/10);
    if(IS_NPC(victim)) cold_effect(victim->in_room,level,dam/2,TARGET_ROOM);
...
/*Sengir adding Mage spells for rotes*/
void spell_rote_quintessence(int sn, int level, CHAR_DATA *ch, void *vo, int target)
{

    if(ch->avatar > 0)
    {
        ch->quintessence += level;
        if(ch->quintessence + ch->paradox > ch->max_quintessence)
            ch->quintessence = ch->max_quintessence - ch->paradox;

        act("You feel the rush of {MQuintessential Energy {xflooding through your body's Pattern!", ch, NULL, NULL, TO_CHAR);
    }
    return;
}
