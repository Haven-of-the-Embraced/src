...
void do_spiritsummoning(CHAR_DATA *ch, char *argument)
{
    MOB_INDEX_DATA *pMobIndex;
    CHAR_DATA *mob;
    AFFECT_DATA af;
    ...
    {
        send_to_char( "You have not even begun to study the mysteries of death and beyond.\n\r", ch );
        return;
    }
    if ( IS_AFFECTED2(ch, AFF2_QUIETUS_BLOODCURSE))
    {
    ...
    if ( ch->pblood < 30 )
    {
        send_to_char( "Calling out to the dead would be too taxing upon your limited blood supply.\n\r", ch );
        return;
    }
    if(ch->pet != NULL)
    {
        send_to_char( "Controlling a being from the {DShadowlands{x is difficult and requires your full concentration.  Dismiss your current minion first.\n\r",ch );
        return;
    }
    ...
    mob->hit = mob->max_hit;
    mob->hitroll = ch->level;
    mob->damroll = mob->level/2;
    ch->pet = mob;

    act( "$n chants and calls to the realm of the dead, bringing forth $N!", ch, NULL, mob, TO_NOTVICT );
    act( "You chant and cajole, stirring the spirits of the dead, calling $N to this plane from its eternal existence.", ch, NULL, mob, TO_CHAR);
...
/    if (ch->pcdata->discipline[QUIETUS] < 5)
    ...
    ch->pblood -= 20;
    dam = 20 - ch->gen;
    sprintf(buf, "%s spits acidic blood at you!\n\r", ch->name);
    send_to_char(buf,victim);
    sprintf(buf, "You spit acidic blood at %s\n\r", victim->name);
    send_to_char(buf,ch);
    act("$n spits acidic blood at $N!", ch,NULL,victim,TO_NOTVICT);
    damage( ch, victim,(ch->level*dam)*3.5, gsn_acidblood, DAM_ACID, TRUE);
    return;
}
*/
...
void do_clotblood(CHAR_DATA *ch, char *argument)
{
   CHAR_DATA *victim;
   char buf[MAX_STRING_LENGTH];
   char arg[MAX_INPUT_LENGTH];
   int chance;
    ...
    victim->pblood -= 10;
/*  damage( ch, victim, 0, TYPE_UNDEFINED, DAM_NONE, FALSE); */
    sprintf(buf, "You convulse as %s clots your blood!\n\r", ch->name);
    send_to_char(buf,victim);
    sprintf(buf, "You force %s's blood to clot!\n\r", victim->name);
    send_to_char(buf,ch);
    act("$N convulses as $n alters their blood!", ch,NULL,victim,TO_NOTVICT);
    multi_hit( victim, ch, TYPE_UNDEFINED );
    return;
}
...
/* Someone fucked up a piece of code.
void do_fleshcraft(CHAR_DATA *ch, char *argument)
{
    char arg [MAX_INPUT_LENGTH];
    ...
    if(ch->pcdata->discipline[VICISSITUDE] < 2)
    {
        send_to_char( "You are not skilled enough in Vicissitude to perform fleshcrafting.\n\r", ch );
        return;
    }
    ...
    if(!str_prefix(arg2,"sex"))
    {
        ...
        if(IS_NPC(mob))
        {
            send_to_char("You cannot bring yourself to alter this poor wretch.\n\r",ch);
            return;
        }
        if(mob->level > ch->level)
        {
            send_to_char("You are too weak to alter this person.\n\r",ch);
            return;
        }
        if(mob->race == race_lookup("vampire") && ch->race == race_lookup("vampire") && mob->gen < ch->gen)
        {
            send_to_char("You should know better than to mess with your elders!\n\r",ch);
            return;
        }
        if(mob->race == race_lookup("methuselah") && mob->race != ch->race)
        {
            send_to_char("Are you insane! How dare you attempt to alter an ancient!\n\r",ch);
            return;
        }
        if(mob->inhumanity == 0 && mob->clan != ch->clan)
        {
            send_to_char("You cannot alter a person who has not sampled the Blood of the Tzimisce or someone who is not of your clan.\n\r",ch);
            return;
        }
        if(str_prefix(arg2,"male") && str_prefix(arg2,"female") && str_prefix(arg2,"it"))
        {
            send_to_char("That is not a valid sex!\n\r",ch);
            return;
        }
        af.where     = TO_AFFECTS;
        af.type      = gsn_vicissitude_fleshcraft;
        af.level     = ch->level;
        af.duration  = ch->level*ch->pcdata->discipline[VICISSITUDE];
        af.location  = APPLY_SEX;
        if(!str_prefix(arg2,"male") && ch->sex == 0)
            af.modifier  = 1;
        else if(!str_prefix(arg2,"male") && ch->sex == 1)
            af.modifier  = 0;
        else if(!str_prefix(arg2,"male") && ch->sex == 2)
            af.modifier  = -1;
        else if(!str_prefix(arg2,"female") && ch->sex == 0)
            af.modifier  = 2;
        else if(!str_prefix(arg2,"female") && ch->sex == 1)
            af.modifier  = 1;
        else if(!str_prefix(arg2,"female") && ch->sex == 2)
            af.modifier  = 0;
        else if(!str_prefix(arg2,"it") && ch->sex == 0)
            af.modifier  = 0;
        else if(!str_prefix(arg2,"it") && ch->sex == 1)
            af.modifier  = -1;
        else if(!str_prefix(arg2,"it") && ch->sex == 2)
            af.modifier  = -2;
        else send_to_char("BUG! INFORM THE IMMORTALS AT ONCE!\n\r",ch);
        af.bitvector = 0;
        affect_to_char( mob, &af );
        switch (mob->sex)
        {
            case 0: act("$n slowly alters $N's sex to neutral.",ch,NULL,mob,TO_ROOM); break;
            case 1: act("$n slowly alters $N's sex to male.",ch,NULL,mob,TO_ROOM); break;
            case 2: act("$n slowly alters $N's sex to female.",ch,NULL,mob,TO_ROOM); break;
            default: send_to_char("BUG! INFORM THE IMMORTALS AT ONCE!\n\r",ch); break;
        }
        return;
    }
    if(ch->pet != NULL)
    ...
    if ( !str_prefix(arg,"hellhound"))
    {
        ...
        sprintf(buf,"hellhound warghoul ghoul %s",mob->name);
        mob->name = str_dup(buf);
        sprintf(buf,"A poor and twisted creature that slighty resembles %s cringes before its master.\n\r",mob->short_descr);
        mob->long_descr = str_dup(buf);
        mob->short_descr = str_dup("A Hellhound");
    ...
    if ( !str_prefix( arg, "szlachta" ) )
    {
        ...
        sprintf(buf,"szlachta warghoul ghoul %s",mob->name);
        mob->name = str_dup(buf);
        sprintf(buf,"A vicious creature that slighty resembles %s snarls in rage at its current form.\n\r",mob->short_descr);
        mob->long_descr = str_dup(buf);
        mob->short_descr = str_dup("A Szlachta Warghoul");
    ...
    if ( !str_prefix( arg, "vozhd" ) )
    {
        ...
        sprintf(buf,"vozhd warghoul ghoul %s",mob->name);
        mob->name = str_dup(buf);
        sprintf(buf,"A vicious bone-armored and spiked version of %s snarls and barely holds back it's urge to kill.\n\r",mob->short_descr);
        mob->long_descr = str_dup(buf);
        mob->short_descr = str_dup("A Vozhd Warghoul");
