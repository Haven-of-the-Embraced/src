...
/*
        ...
        fprintf( fp, "\n");
*/
        fprintf( fp, "Disciplines %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    if (ch->version < 6)
    {
        if (IS_VAMP(ch) && (ch->level < 101)) {
            if (ch->gen != 10) {
                if (ch->gen == 12)
                    ch->gen = 10;
                if (ch->gen < 10)
                    ch->pcdata->csbackgrounds[CSBACK_GENERATION] = (10-ch->gen);
            }
        }
    ...
    if (ch->version <= 7)
{
    if (ch->pcdata->csvirtues[CSVIRT_CONSCIENCE] == ch->pcdata->csbackgrounds[CSBACK_STATUS])
        ch->pcdata->csbackgrounds[CSBACK_STATUS] = 0;
    ch->version = 8;
}
if (ch->version <= 8)
{
    ch->pcdata->pwd = str_dup(crypt (ch->pcdata->pwd, ch->name));
    ch->version = 9;
    }
