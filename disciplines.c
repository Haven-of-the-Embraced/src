...
void do_touch( CHAR_DATA *ch, char *argument )
{
    ...
    switch ( obj->item_type )
    {
        ...
        case ITEM_WEAPON:
            sprintf(buf, "{m()  {MA fierce warrior takes a stance, wielding this {D({r+%d{D){M ", obj->value[1] / 20);
            send_to_char(buf, ch);
            switch (obj->value[0])
            {
    ...
    if ( IS_SET(obj->extra_flags, ITEM_BLESS) )
        send_to_char("{m()  {MA {W[holy individual]{M prays intently, heartfelt and pure.                   {m(){x\n\r", ch);
    ...
    if ( IS_SET(obj->extra_flags, ITEM_IS_PRESERVE) )
        send_to_char("{m()  {MA variety of food is being {y[dipped]{M into this object.                     {m(){x\n\r", ch);
...
void do_telepathy( CHAR_DATA *ch, char *argument )
{
    ...
    if (success == 0)
    {
        act("You fail to open your mind to hear thoughts around you.",ch,NULL,victim,TO_CHAR);
        WAIT_STATE(ch, 15);
        return;
    }
...
void do_essence(CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("You have already altered yourself to another form!\n\r", ch);
        return;
    }
    ...
    af.where     = TO_IMMUNE;
    af.type      = gsn_vicissitude_essence;
    af.level     = ch->level;
    af.duration  = -1;
    af.location  = 0;
    af.modifier  = 0;
