...
void do_pstat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Name: %s (%s[%d])     Clan: %s\n\r",
    victim->name,
    IS_IMMORTAL(victim) ? "an Immortal" : "a Player", victim->level, clan_table[victim->clan].name);
    send_to_char( buf, ch );

    sprintf( buf,"Race: %s  Class: %s  Sex: %s  Align: %d  Exp: %d  Room: %d\n\r",
    ...
    send_to_char("                    {g-------Charsheet Stats------{x\n\r",ch);
    sprintf(buf, "Str: %d  Dex: %d  Sta: %d  Cha: %d  Man: %d  App: %d  Per: %d  Int: %d  Wits: %d\n\r",
    get_attribute(victim,STRENGTH), get_attribute(victim,DEXTERITY), get_attribute(victim,STAMINA),
    get_attribute(victim,CHARISMA), get_attribute(victim,MANIPULATION), get_attribute(victim,APPEARANCE),
    get_attribute(victim,PERCEPTION), get_attribute(victim,INTELLIGENCE), get_attribute(victim,WITS));
    send_to_char(buf,ch);


    if(IS_IMMORTAL(victim))
    {
        send_to_char("                        {g-------Imm Info-------{x\n\r",ch);
    ...
    if (victim->avatar > 0)
    {
        send_to_char("                       {g------Mage Info-------{x\n\r",ch);
        sprintf(buf,"Tradition: %s  Mentor: %s  Apprentice: %s  Rank: %s  Spoint: %d\n\rAvatar: %d  Arete: %d  Quint: %d  Paradox: %d\n\r",
        capitalize(tradition_table[victim->tradition].name),
...
void do_string( CHAR_DATA *ch, char *argument )
{
    ...
    /* string something */
        ...
        if (!str_cmp, (arg2, "wiziname") && IS_IMMORTAL(victim))
        {
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
    ...
    /*Format it up.*/
        ...
        sendch("{D     L          C{x\n\r", ch);
        sendch("{D     E   R      L       M    M    I     N        H{x\n\r", ch);
        sendch("{D  S  V   A      A       A    O    D     A        O{x\n\r", ch);
        sendch("{D  E  E   C      S   H   N    V    L     M        S{x\n\r", ch);
        sendch("{D  X  L   E      S   P   A    E    E     E        T{x\n\r", ch);
