...
// result added to shut up compiler. Dumb compiler warning - Ugha
void do_test2(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf,"ls ../player/> %s", LS_FILE);
    result = system(buf);
/*  sprintf(buf,"Ls file made.");
    send_to_char(buf,ch);
*/
    if ((fp = fopen(LS_FILE,"r")) == NULL)
    {
        bug("Couldn't open ls file.",0);
        fclose(fp);
        return;
    }
/*  sprintf(buf,"Ls file open.");
    send_to_char(buf,ch);
*/
    name = "START";
    while(str_cmp(name, "END"))
    {
        count++;
/*      if(count > 1200) break; */
        name = feof(fp) ? "END" : fread_string_eol( fp );
        if(!str_cmp(name,"END")) break;
        sprintf(buf,"../player/%s",name);
        if(stat(buf,&file))
        {

            filetime = *localtime(&file.st_mtime);

            if (filetime.tm_year < nowtime.tm_year)
            {
                total = filetime.tm_yday - nowtime.tm_yday;
                total = (365 - total);
            }
            else
                total = nowtime.tm_yday - filetime.tm_yday;

            if (total > 182)
            {
                deleteme = TRUE;

/*              sprintf(buf,"The last mod time of the file %s is %s", name, (char *)asctime(localtime(&(file.st_mtime))));
                send_to_char(buf,ch);
                sprintf(buf,"The day on the file is %d. The current day is %d\n\r",filetime.tm_yday, nowtime.tm_yday);
                send_to_char(buf,ch);
                sprintf(buf,"Condition of Deleteme is %s", deleteme ? "TRUE" : "FALSE");
                send_to_char(buf,ch);
*/
            }
        }
...
{
    char string[50][MAX_STRING_LENGTH];
    char finished[MAX_STRING_LENGTH];
    char word[50][MAX_STRING_LENGTH];
    char arg1[MAX_INPUT_LENGTH];
    int length, i, words = 0, line = 0;
    ...
    for(i = 0;;i++)
    {
        argument = one_argument(argument,word[i]);
        if(word[i][0] == '\0') break;
        words++;
    }
    ...
    for(i = 0;i < words;i++)
    {
        if((strlen(string[line])+strlen(word[i])) < length)
        {
            if(i) strcat(string[line]," ");
            strcat(string[line],word[i]);
        }
        else
        {
            string[line][strlen(string[line])] = '\0';
            line++;
        }
    }
...
void do_test(CHAR_DATA *ch, char *argument)
{
    ...
    char word[50][MAX_STRING_LENGTH];
    char sentence[50][MAX_STRING_LENGTH];
    char arg1[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
    int i, length, words = 0, line = 0;
    ...
    for(i = 0;i < words;i++)
    {
        if(strlen(str2)+strlen(word[i])+1 > length)
        {
            line++;
            strcpy(str2,"\0");
        }
    ...
    sprintf(buf,"There are supposed to be %d lines:\n\r",line+1);
    send_to_char(buf,ch);
...
void do_test(CHAR_DATA *ch, char *argument)
{
    ...
    if(arg2[0] != '\0' && arg3[0] != '\0' && !str_cmp(arg2,"add"))
    {
        if(!is_number(arg3))
            return;
        if(atoi(arg3) > 100 || atoi(arg3) < -100)
            return;

        add_like(victim,ch,atoi(arg3));

    }


    if(get_like(victim,ch) == 0)
        add_like(victim,ch,0);

    sprintf(buf,"My like level for you is %d",get_like(victim,ch));
    do_function(victim, &do_say, buf );
